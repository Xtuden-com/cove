...
@pytest.mark.parametrize(('source_filename', 'expected_text', 'conversion_successful'), [
    ...
    ('badfile_all_validation_errors.json', [
        ...
        '0 should be a JSON object',
        'amountAwarded should be a number. Check that the value is not null, and doesn’t contain any characters other than 0-9 and dot (.). Number values should not be in quotes.',
        'plannedDates should be a JSON array',
        'title should be a string. Check that the value is not null, and has quotes at the start and end. Escape any quotes in the value with \ (more info)',
    ...
    ('badfile_all_validation_errors.xlsx', [
        ...
        'id is missing but required within recipientOrganization (more info)',
        'Date is not in the correct format (more info)',
        'Amount Awarded should be a number. Check that the value is not null, and doesn’t contain any characters other than 0-9 and dot (.). Number values should not be in quotes.',
        'Invalid \'uri\' found (more info)',
        'Currency contains an unrecognised value. Check the related codelist for allowed code values.',
        '[] is too short. You must supply at least one value, or remove the item entirely (unless it’s required).',
    ], True),
])
